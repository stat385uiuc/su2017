...
- **ui.R:** is responsible for providing the _frontend_
 user interface (ui) for the shiny application.
...
```{r backend_v_frontend, out.width = "250px", fig.retina = NULL, fig.align='center', cache = TRUE, echo = FALSE}
...
Observers perform _actions_ when reactive values or expressions change.

There are two forms of observers:

- **Implicit**: Triggered whenever _any_ reactive values or expressions are encountered: 

```r
observe({...})
```

- **Explicit**: Triggered only when a specific reactive value or expression changes. 
  Any other reactive values outside of the observed event are ignored. 
...
shinyServer(function(input, output) {
  ...
  # Whenever input$x or input$y change, execute
  observe({
    cat("input$x has", input$x, "and input$y has", input$y ,"\n")
  })
  
  # Execute only when form submission changes
...
Avoid making the mistake interpreting the code as triggering the update  `view` with
the results.
