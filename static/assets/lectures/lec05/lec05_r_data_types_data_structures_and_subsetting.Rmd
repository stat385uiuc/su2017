...
```{r}
a = runif(5)      # Five random numbers in (0, 1)
...
**Note:** Merging `numeric` with `character` yields `character`

### Converting Between Homogeneous Types

Sometimes, you may have data that has been given to you in `character` form but
really should be `numeric` and vice versa.
...
### Sequence Operator and Arithmetic Operations
