...
1. Decrease the probability of an error
    - Update code in one central location instead of all over. 
...
## Case Study: Linear Regression Diagnostics

### Defining quantities $TSS$, $RSS$, and $FSS$

Often, when working with linear regression, we want to compute three different 
summations to compute other useful diagnostic information:
...
### Dynamic Function Declaration: $TSS$, $RSS$, $FSS$ Example
...
### Exploiting a Relationship: $TSS$, $RSS$, $FSS$ Example
...
### Reusing Functions: $TSS = RSS + FSS$ Relationship
...
### Creating Sample Data for Functions 
...
**Note:** This behavior can be problematic. Try to keep your scope limited.
...
To access the cache the in-between levels, you will need to declare the 
memoization function name within the function. e.g. 
...
Internal memoization yields a higher speed gain compared to external.
